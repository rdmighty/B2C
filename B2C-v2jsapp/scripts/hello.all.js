	...
	login: function() {
		...
		if (opts.display === 'none') {
		    ...
		    utils.iframe(url, function (err) {
		        if (err) {
		            var response = error('blocked', 'Iframe was blocked')
...
hello.utils.extend(hello.utils, {
	...
	iframe: function (src, cb) {
	    ...
	    ifr.onload = function () {
	        // if same origin fails, we will not be able
	        // to access the contentDocument.location
	        // this seems to be a only way to find out
	        // something went wrong
	        try {
	            loc = ifr.contentDocument.location
	        } catch (e) {
	            if ('function' === typeof cb) {
	                cb('Iframe load failed.');
	            }
	        }
	    }
